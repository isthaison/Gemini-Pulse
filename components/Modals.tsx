    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
      <div className="bg-zinc-900 border border-white/10 rounded-2xl md:rounded-[2.5rem] p-6 md:p-10 max-w-2xl w-full shadow-2xl max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-2xl md:text-3xl font-bold tracking-tight">Diagnostics</h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-white/5 rounded-xl">
                ...
                <div className="font-mono text-xs">{peerId || 'Not connected'}</div>
              </div>
              <div className="p-3 bg-white/5 rounded-xl">
                <div className="text-sm text-zinc-400">Signaling State</div>
                <div className={`font-mono text-xs ${signalingState === 'ready' ? 'text-green-400' : signalingState === 'disconnected' ? 'text-yellow-400' : 'text-red-400'}`}>
                  {signalingState}
                </div>
              </div>
              <div className="p-3 bg-white/5 rounded-xl">
              ...
              <div className="p-3 bg-white/5 rounded-xl">
                ...
                <div className="font-mono text-xs">{isCalling ? 'Yes' : 'No'}</div>
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="max-h-40 overflow-y-auto space-y-1">
              {messages.filter(m => m.sender === 'system').slice(-10).map((msg, idx) => (
                <div key={idx} className="text-xs text-zinc-400 bg-white/5 p-2 rounded">
                  {msg.content}
                </div>
              ))}
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
        ...
        <p className="text-sm text-zinc-400 mb-4">You have an incoming call from <code className="font-mono text-xs">{pendingIncomingCall}</code></p>
        <div className="flex gap-3">
          <button onClick={onReject} className="flex-1 py-2 bg-red-600 hover:bg-red-500 rounded-xl text-white font-bold">Decline</button>
          <button onClick={onAccept} className="flex-1 py-2 bg-green-600 hover:bg-green-500 rounded-xl text-white font-bold">Accept</button>
        </div>
      </div>
    </div>
  );
