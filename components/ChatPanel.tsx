...
const ChatPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 custom-scrollbar">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-20">
            <Bot
              size={36}
              className="md:w-12 md:h-12 mb-3 md:mb-4 stroke-[1px]"
            />
            <p className="text-[10px] md:text-xs font-bold uppercase tracking-widest">
              Signal Terminal Ready
            </p>
            <p className="text-[9px] md:text-[10px] mt-1 md:mt-2 max-w-40 md:max-w-45">
              Start chatting with peers to coordinate your video call.
            </p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              ...
              >
                <div className="max-w-[90%] md:max-w-[85%]">
                  <div
                    className={`flex items-center gap-1 md:gap-2 mb-1 md:mb-1.5 px-1 md:px-2 ${
                  ...
                  >
                    ...
                    <span className="text-[7px] md:text-[8px] text-white/10">
                  ...
                  <div
                    className={`px-3 md:px-5 py-2 md:py-3 rounded-lg md:rounded-3xl text-xs md:text-sm leading-relaxed ${
                      isSelf
                        ? "bg-blue-600 text-white rounded-tr-none shadow-xl shadow-blue-900/20"
                        : isSystem
                        ? "bg-white/6 text-zinc-200 italic"
      ...
      {/* Input */}
      <div className="p-4 md:p-6 bg-white/1 border-t border-white/5">
        <form
        ...
        >
          <input
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-xl md:rounded-2xl px-3 md:px-5 py-2.5 md:py-3.5 text-xs md:text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-blue-500/50 focus:bg-white/10 transition-all"
          />
          <button
            type="submit"
