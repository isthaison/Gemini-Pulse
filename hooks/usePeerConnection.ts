...
interface UsePeerConnectionProps {
  ...
  // Updated to support functional updates (prev => next) to match usage in hook
...
export const usePeerConnection = ({
  ...
  const setupDataConnection = useCallback((peerId: string, conn: DataConnection) => {
    conn.on('open', () => {
  ...
  const initPeer = useCallback(() => {
    ...
    peer.on('call', (call) => {
      ...
      call.on('stream', (remoteStream) => {
        ...
        // Using updater function for state management to ensure consistency
    ...
    peer.on('connection', (conn) => {
      ...
      // Update the remote peer's data connection using updater
      onRemotePeersUpdate(prev => {
  ...
  }, [onPeerIdGenerated, onRemotePeersUpdate, onConnectedPeersCountChange, setupDataConnection]);

  const callPeers = useCallback((remoteIds: string[], localStream: MediaStream) => {
